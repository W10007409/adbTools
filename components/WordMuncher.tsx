...
const QUIZ_LIST: Quiz[] = [
  { image: '?뜋', word: 'Apple', hint: '?ш낵', category: 'Fruits' },
  { image: '?뜉', word: 'Banana', hint: '諛붾굹??, category: 'Fruits' },
  { image: '?맯', word: 'Dog', hint: '媛뺤븘吏', category: 'Animals' },
  { image: '?맩', word: 'Cat', hint: '怨좎뼇??, category: 'Animals' },
  { image: '?슅', word: 'Car', hint: '?먮룞李?, category: 'Transport' },
  { image: '?뙙', word: 'Sun', hint: '?쒖뼇', category: 'Nature' },
  { image: '?뱴', word: 'Book', hint: '梨?, category: 'Objects' },
  { image: '??, word: 'Ball', hint: '怨?, category: 'Sports' }
];

const TREAT_CONFIG: Record<TreatType, { emoji: string; color: string }> = {
  cupcake: { emoji: '?쭅', color: '#ff80ab' },
  cookie: { emoji: '?뜧', color: '#a1887f' },
  donut: { emoji: '?뜦', color: '#f06292' },
  icecream: { emoji: '?뜣', color: '#81d4fa' },
  cake: { emoji: '?뜲', color: '#ffd54f' },
  cherry: { emoji: '?뜏', color: '#ef5350' }
};
...
const WordMuncher: React.FC = () => {
  ...
  const [advice, setAdvice] = useState<string>("?⑥뼱瑜?李얠븘蹂댁꽭??");
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#0f172a] z-50">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-white tracking-widest uppercase">?곸뼱 援먯떎 ?낆옣 以?..</h1>
          </div>
        </div>
      )}
